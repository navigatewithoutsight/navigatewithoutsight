...
//on off 
static void active_on(void)
{
    gpio_set_level(ACTIVE_BUZZER_PIN, 1);
}

static void active_off(void)
{
    gpio_set_level(ACTIVE_BUZZER_PIN, 0);
}

static void passive_on(void)
{
    gpio_set_level(PASSIVE_BUZZER_PIN, 1);
}

static void passive_off(void)
{
    gpio_set_level(PASSIVE_BUZZER_PIN, 0);
}
...
static void play_distance_pattern(void)
{
    ...
    if (distance_cm > 150)      wait_ms = 1000;  // far
    else if (distance_cm > 80)  wait_ms = 500;
    else if (distance_cm > 50)  wait_ms = 250;
    else                        wait_ms = 150;   // close
