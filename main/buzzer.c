...
static void passive_pwm_init(void) {
  ledc_timer_config_t timer = {.speed_mode = PASSIVE_LEDC_MODE,
  ...
  ledc_channel_config_t ch = {.gpio_num = PASSIVE_BUZZER_PIN,
...
void buzzer_init(void) {
  gpio_config_t io = {.pin_bit_mask = 1ULL << ACTIVE_BUZZER_PIN,
...
void buzzer_set_speed(float speed_mps) {
  s_speed_mps = speed_mps;
  if (s_speed_mps < 0)
    s_speed_mps = 0;
}

void buzzer_set_distance(int distance_mm, int is_valid) {
...
void buzzer_set_direction(int direction) { s_direction = direction; }

void buzzer_set_mode_event(int event_code) { s_mode_event = event_code; }

void buzzer_set_error(int has_error) { s_error_on = (has_error != 0); }

/* Distance to beep timing */
static uint32_t distance_to_period_ms(uint16_t distance_mm) {
  ...
  if (distance_mm >= 1500) {
    ...
    base_period = 150 + (uint32_t)((1000 - 150) * (distance_mm - 300) / 1200);
  }

  float speed_factor = 1.0f + (s_speed_mps * 0.4f);
  uint32_t adjusted_period = (uint32_t)(base_period / speed_factor);

  if (adjusted_period < 50)
    adjusted_period = 50;
...
float buzzer_get_hz(void) {
  return 1000.0 / distance_to_period_ms(s_distance_mm);
}
...
static void service_passive_mode_event(int64_t tnow) {
  if (s_mode_event == 0)
    return;
  if (s_passive_event_end_us > tnow)
    return;
  ...
  if (s_mode_event == 1) {
    freq = 2500;
    dur_ms = 150;
  } else if (s_mode_event == 2) {
    freq = 1200;
    dur_ms = 180;
  } else if (s_mode_event == 3) {
    freq = 1800;
    dur_ms = 120;
  }
...
static void service_passive_stop(int64_t tnow) {
  if (s_passive_is_on && s_passive_event_end_us > 0 &&
...
static void service_active_error(int64_t tnow) {
  ...
  bool on = (t < 100000) || (t >= 200000 && t < 300000) ||
            (t >= 400000 && t < 500000);

  if (on != s_active_is_on)
    active_set(on);
}
...
static void service_active_distance(int64_t tnow) {
  ...
  // ESP_LOGI(TAG, "[DEBUG] distance_cm=%d, speed=%.2f m/s", s_distance_mm,
  //          s_speed_mps);
  if (s_distance_mm > 0 && s_distance_mm < 300) {
    if (!s_active_is_on)
      active_set(true);
    return;
  }

  uint32_t period_ms = distance_to_period_ms(s_distance_mm);
  // ESP_LOGI(TAG, "[DEBUG] period_ms=%u", (int)period_ms);
...
void buzzer_iteration_main(int err, int event, int direction_arg,
  ...
  if (event)
    buzzer_set_mode_event(event);
  if (direction_arg)
    buzzer_set_direction(direction_arg);
