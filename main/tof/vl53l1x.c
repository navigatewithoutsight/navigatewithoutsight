// VL53L0X control
// Copyright © 2021 Adrian Kennard, Andrews & Arnold Ltd, and original authors.
// See LICENCE file for details. GPL 3.0 Based on
...
enum {
  ...
  RANGE_CONFIG__TIMEOUT_MACROP_A =
      0x005E, // added by Pololu for 16-bit accesses
  ...
  RANGE_CONFIG__TIMEOUT_MACROP_B =
      0x0061, // added by Pololu for 16-bit accesses
...
static inline uint8_t vl53l1x_checkTimeoutExpired(vl53l1x_t *v) {
  return (v->io_timeout > 0) &&
         ((uint16_t)(millis() - v->timeout_start_ms) > v->io_timeout);
}

uint32_t vl53l1x_timeoutMicrosecondsToMclks(uint32_t timeout_us,
...
static uint32_t vl53l1x_timeoutMclksToMicroseconds(uint32_t timeout_mclks,
                                                   uint32_t macro_period_us) {
...
void vl53l1x_setupManualCalibration(vl53l1x_t *v) {
  ...
  v->saved_vhv_timeout =
      vl53l1x_readReg(v, VHV_CONFIG__TIMEOUT_MACROP_LOOP_BOUND);
  ...
  vl53l1x_writeReg(
      v, VHV_CONFIG__TIMEOUT_MACROP_LOOP_BOUND,
      (v->saved_vhv_timeout & 0x03) +
          (3
           << 2)); // tuning parm default (LOWPOWERAUTO_VHV_LOOP_BOUND_DEFAULT)

  // "override phasecal"
  vl53l1x_writeReg(v, PHASECAL_CONFIG__OVERRIDE, 0x01);
  vl53l1x_writeReg(v, CAL_CONFIG__VCSEL_START,
                   vl53l1x_readReg(v, PHASECAL_RESULT__VCSEL_START));
}
...
uint8_t vl53l1x_setMeasurementTimingBudget(vl53l1x_t *v, uint32_t budget_us) {
  ...
  macro_period_us = vl53l1x_calcMacroPeriod(
      v, vl53l1x_readReg(v, RANGE_CONFIG__VCSEL_PERIOD_A));
  ...
  uint32_t phasecal_timeout_mclks =
      vl53l1x_timeoutMicrosecondsToMclks(1000, macro_period_us);
  ...
  vl53l1x_writeReg16Bit(
      v, MM_CONFIG__TIMEOUT_MACROP_A,
      vl53l1x_encodeTimeout(
          vl53l1x_timeoutMicrosecondsToMclks(1, macro_period_us)));

  // "Update Range Timing A timeout"
  vl53l1x_writeReg16Bit(
      v, RANGE_CONFIG__TIMEOUT_MACROP_A,
      vl53l1x_encodeTimeout(vl53l1x_timeoutMicrosecondsToMclks(
          range_config_timeout_us, macro_period_us)));

  // "Update Macro Period for Range B VCSEL Period"
  macro_period_us = vl53l1x_calcMacroPeriod(
      v, vl53l1x_readReg(v, RANGE_CONFIG__VCSEL_PERIOD_B));

  // "Update MM Timing B timeout"
  // (See earlier comment about MM Timing A timeout.)
  vl53l1x_writeReg16Bit(
      v, MM_CONFIG__TIMEOUT_MACROP_B,
      vl53l1x_encodeTimeout(
          vl53l1x_timeoutMicrosecondsToMclks(1, macro_period_us)));

  // "Update Range Timing B timeout"
  vl53l1x_writeReg16Bit(
      v, RANGE_CONFIG__TIMEOUT_MACROP_B,
      vl53l1x_encodeTimeout(vl53l1x_timeoutMicrosecondsToMclks(
          range_config_timeout_us, macro_period_us)));
...
static i2c_cmd_handle_t Read(vl53l1x_t *v, uint16_t reg) { // Set up for read
...
static i2c_cmd_handle_t Write(vl53l1x_t *v, uint16_t reg) { // Set up for write
  i2c_cmd_handle_t i = i2c_cmd_link_create();
  i2c_master_start(i);
  i2c_master_write_byte(i, (v->address << 1), 1);
  i2c_master_write_byte(i, reg >> 8, 1);
...
uint16_t vl53l1x_readReg16Bit(vl53l1x_t *v, uint16_t reg) {
  ...
  v->err = Done(v, i);
  VL53L1X_LOG(TAG, "R %04X=%02X%02X %s", reg, buf[0], buf[1],
              esp_err_to_name(v->err));
...
uint32_t vl53l1x_readReg32Bit(vl53l1x_t *v, uint16_t reg) {
  ...
  v->err = Done(v, i);
  VL53L1X_LOG(TAG, "R %04X=%02X%02X%02X%02X %s", reg, buf[0], buf[1], buf[2],
              buf[3], esp_err_to_name(v->err));
...
void vl53l1x_readMulti(vl53l1x_t *v, uint16_t reg, uint8_t *dst,
                       uint8_t count) {
  i2c_cmd_handle_t i = Read(v, reg);
  if (count > 1)
...
void vl53l1x_writeMulti(vl53l1x_t *v, uint16_t reg, uint8_t const *src,
                        uint8_t count) {
  i2c_cmd_handle_t i = Write(v, reg);
...
vl53l1x_t *vl53l1x_config(int8_t port, int8_t scl, int8_t sda, int8_t xshut,
  ...
  if (i2c_param_config(port, &config)) { // Config failed
...
const char *vl53l1x_init(vl53l1x_t *v) {
  ...
  // give it some time to boot; otherwise the sensor NACKs during the
  // vl53l1x_readReg(v)
  usleep(100000);
  ...
  // correctly
  while ((vl53l1x_readReg(v, FIRMWARE__SYSTEM_STATUS) & 0x01) == 0 ||
         v->err != 0) {
  ...
  if (v->io_2v8) {
    vl53l1x_writeReg(v, PAD_I2C_HV__EXTSUP_CONFIG,
                     vl53l1x_readReg(v, PAD_I2C_HV__EXTSUP_CONFIG) | 0x01);
  }

  // store oscillator info for later use
  v->fast_osc_frequency =
      vl53l1x_readReg16Bit(v, OSC_MEASURED__FAST_OSC__FREQUENCY);
  ...
  vl53l1x_writeReg16Bit(v, DSS_CONFIG__TARGET_TOTAL_RATE_MCPS,
                        TargetRate); // should already be this value after reset
  vl53l1x_writeReg(v, GPIO__TIO_HV_STATUS, 0x02);
  vl53l1x_writeReg(v, SIGMA_ESTIMATOR__EFFECTIVE_PULSE_WIDTH_NS,
                   8); // tuning parm default
  vl53l1x_writeReg(v, SIGMA_ESTIMATOR__EFFECTIVE_AMBIENT_WIDTH_NS,
  ...
  vl53l1x_writeReg(v, ALGO__CONSISTENCY_CHECK__TOLERANCE,
  ...
  vl53l1x_writeReg16Bit(v, RANGE_CONFIG__SIGMA_THRESH,
                        360); // tuning parm default
  vl53l1x_writeReg16Bit(v, RANGE_CONFIG__MIN_COUNT_RATE_RTN_LIMIT_MCPS,
  ...
  vl53l1x_writeReg16Bit(v, ALGO__PART_TO_PART_RANGE_OFFSET_MM,
                        vl53l1x_readReg16Bit(v, MM_CONFIG__OUTER_OFFSET_MM) *
                            4);
...
uint8_t vl53l1x_setDistanceMode(vl53l1x_t *v, vl53l1x_DistanceMode mode) {
  ...
  case VL53L1X_Medium:
    ...
    vl53l1x_writeReg(v, SD_CONFIG__INITIAL_PHASE_SD0,
                     10); // tuning parm default
    vl53l1x_writeReg(v, SD_CONFIG__INITIAL_PHASE_SD1,
                     10); // tuning parm default
...
uint32_t vl53l1x_getMeasurementTimingBudget(vl53l1x_t *v) {
  ...
  // "Update Macro Period for Range A VCSEL Period"
  uint32_t macro_period_us = vl53l1x_calcMacroPeriod(
      v, vl53l1x_readReg(v, RANGE_CONFIG__VCSEL_PERIOD_A));

  // "Get Range Timing A timeout"

  uint32_t range_config_timeout_us = vl53l1x_timeoutMclksToMicroseconds(
      vl53l1x_decodeTimeout(
          vl53l1x_readReg16Bit(v, RANGE_CONFIG__TIMEOUT_MACROP_A)),
      macro_period_us);
...
void vl53l1x_setROISize(vl53l1x_t *v, uint8_t width, uint8_t height) {
  ...
  vl53l1x_writeReg(v, ROI_CONFIG__USER_ROI_REQUESTED_GLOBAL_XY_SIZE,
                   (height - 1) << 4 | (width - 1));
}
...
void vl53l1x_getROISize(vl53l1x_t *v, uint8_t *width, uint8_t *height) {
  uint8_t reg_val =
      vl53l1x_readReg(v, ROI_CONFIG__USER_ROI_REQUESTED_GLOBAL_XY_SIZE);
  *width = (reg_val & 0xF) + 1;
  *height = (reg_val >> 4) + 1;
}
...
void vl53l1x_startContinuous(vl53l1x_t *v, uint32_t period_ms) {
  ...
  vl53l1x_writeReg32Bit(v, SYSTEM__INTERMEASUREMENT_PERIOD,
                        period_ms * v->osc_calibrate_val);

  vl53l1x_writeReg(v, SYSTEM__INTERRUPT_CLEAR,
...
void vl53l1x_stopContinuous(vl53l1x_t *v) {
  ...
  if (v->saved_vhv_timeout != 0) {
    vl53l1x_writeReg(v, VHV_CONFIG__TIMEOUT_MACROP_LOOP_BOUND,
                     v->saved_vhv_timeout);
  }
...
const char *vl53l1x_rangeStatusToString(vl53l1x_t *v,
...
void vl53l1x_updateDSS(vl53l1x_t *v) {
  ...
  if (spadCount != 0) {
    ...
    if (totalRatePerSpad != 0) {
      ...
      // "override DSS config"
      vl53l1x_writeReg16Bit(v, DSS_CONFIG__MANUAL_EFFECTIVE_SPADS_SELECT,
                            requiredSpads);
...
void vl53l1x_getRangingData(vl53l1x_t *v) {
  ...
  // appears to scale the result by 2011/2048, or about 98% (with the 1024 added
  // for proper rounding).
  ...
  // from SetSimpleData()
  v->ranging_data.peak_signal_count_rate_MCPS = countRateFixedToFloat(
      v->results.peak_signal_count_rate_crosstalk_corrected_mcps_sd0);
  v->ranging_data.ambient_count_rate_MCPS =
      countRateFixedToFloat(v->results.ambient_count_rate_mcps_sd0);
}
