...
turn_direction_t GY87_detect_turn(float gyro_z) {
    if (gyro_z > TURN_THRESHOLD) return TURN_RIGHT;
    if (gyro_z < -TURN_THRESHOLD) return TURN_LEFT;
    return TURN_NONE;
}
...
void GY87_update_cadence_and_speed(void) {
    ...
    if ((now - cadence_window_start_us) > (CADENCE_WINDOW_MS * 1000)) {
        ...
        ESP_LOGI("GAIT",
                 "Cadence: %.1f spm | Estimated speed: %.2f m/s",
                 cadence_spm,
                 speed_mps);
    ...
    while (1) {
        if (GY87_read_gyro_z(&gz) == 0) {
            ...
            if (dir == TURN_LEFT) {
                ...
                ssd1306_draw_text(20, 3, "LEFT");
            }
            else if (dir == TURN_RIGHT) {
                ESP_LOGI(TAG, "Turning RIGHT (%.2f °/s)", gz);
                ssd1306_draw_text(20, 3, "RIGHT");
            }
            else {
                ESP_LOGI(TAG, "No turn (%.2f °/s)", gz);
                ssd1306_draw_text(10, 3, "NO TURN");
            }
        }
