...
static void ssd1306_cmd(uint8_t cmd) {
  ...
  i2c_master_write_to_device(I2C_PORT, SSD1306_ADDR, buf, 2,
                             pdMS_TO_TICKS(100));
}
...
void ssd1306_init(void) {
  const uint8_t cmds[] = {0xAE, 0xD5, 0x80, 0xA8, 0x3F, 0xD3, 0x00,
                          0x40, 0x8D, 0x14, 0x20, 0x00, 0xA1, 0xC8,
                          0xDA, 0x12, 0x81, 0x7F, 0xA4, 0xA6, 0xAF};
...
void ssd1306_update(void) {
  for (uint8_t page = 0; page < 8; page++) {
    ...
    i2c_master_write_to_device(I2C_PORT, SSD1306_ADDR, data, sizeof(data),
                               pdMS_TO_TICKS(100));
  }
}
...
void ssd1306_render_dashboard(int turn_dir, int turn_deg, int spm,
  ...
  if (xSemaphoreTake(mutex, portMAX_DELAY) == pdTRUE) {
    ...
    if (turn_dir < 0)
      ssd1306_draw_text_xy(2, top_h + 6, "LEFT");
    else if (turn_dir > 0)
      ssd1306_draw_text_xy(2, top_h + 6, "RIGHT");
    else
      ssd1306_draw_text_xy(2, top_h + 6, "NONE");
    ...
    // ESP_LOGI("Screen", "Release lock");
  } else {
    // ESP_LOGI("Screen", "Can't aquire lock");
    return;
  }
}
